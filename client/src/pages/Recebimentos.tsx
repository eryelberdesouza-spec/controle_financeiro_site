...
type FormData = {
  ...
  descricao: string;
  tipoRecebimento: "Pix" | "Boleto" | "Transferência" | "Cartão" | "Dinheiro" | "Outro";
  valorTotal: string;
...
const defaultForm: FormData = {
  numeroContrato: "", nomeRazaoSocial: "", descricao: "",
  tipoRecebimento: "Pix", valorTotal: "", valorEquipamento: "0",
  valorServico: "0", quantidadeParcelas: 1, parcelaAtual: 1,
...
export default function Recebimentos() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.nomeRazaoSocial || !form.valorTotal || !form.dataVencimento) {
  ...
  const handleEdit = (r: any) => {
    setEditId(r.id);
    setForm({
      numeroContrato: r.numeroContrato ?? "", nomeRazaoSocial: r.nomeRazaoSocial ?? "",
      descricao: r.descricao ?? "", tipoRecebimento: r.tipoRecebimento ?? "Pix",
      valorTotal: String(r.valorTotal ?? ""), valorEquipamento: String(r.valorEquipamento ?? "0"),
      valorServico: String(r.valorServico ?? "0"), quantidadeParcelas: r.quantidadeParcelas ?? 1,
  ...
  const filtered = recebimentos.filter(r => {
    const matchSearch = !search || r.nomeRazaoSocial.toLowerCase().includes(search.toLowerCase()) || (r.numeroContrato ?? "").includes(search);
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <Input placeholder="Buscar por nome ou contrato..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9" />
          </div>
                      ...
                      <tr key={r.id} className="border-b hover:bg-muted/30 transition-colors">
                        ...
                        <td className="p-3 text-right">
                          <div className="flex justify-end gap-1">
                            ...
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => { if (confirm("Remover este recebimento?")) deleteMutation.mutate({ id: r.id }); }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select value={form.tipoRecebimento} onValueChange={v => setForm(f => ({ ...f, tipoRecebimento: v as any }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {["Pix", "Boleto", "Transferência", "Cartão", "Dinheiro", "Outro"].map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div>
